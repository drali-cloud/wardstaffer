...
const DashboardView = React.memo(({ staffing, user }: { staffing: any, user: AuthUser }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {user.role === 'admin' && (<div className="technical-card p-8 border-blue-100 ring-1 ring-blue-50 flex items-center justify-between gap-8"><div><h2 className="text-xs font-bold text-slate-800 uppercase tracking-widest mb-2 flex items-center gap-2"><Save className="w-4 h-4 text-blue-600" /> New Monthly Dispatch</h2><p className="text-xs text-slate-400">Initialize the personnel pool for the next rotation cycle.</p></div><div className="flex gap-4"><input type="month" className="text-sm p-3 bg-slate-50 border border-slate-200 rounded-xl" value={targetPeriod} onChange={(e) => setTargetPeriod(e.target.value)} /><button disabled={staffing.syncing} onClick={() => { if(confirm(`Generate for ${targetPeriod}?`)) staffing.generateMonthlyDispatch(targetPeriod); }} className="btn-primary px-8">Initiate Rotation</button></div></div>)}
    </div>
  );
});
